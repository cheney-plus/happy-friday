...
const getModelProviderIcon = (provider: string): string => {
  ...
  return found?.icon || new URL('@/assets/images/其他模型.png', import.meta.url).href;
};

const getModelDescription = (model: { provider: string; modelName: string }): string => {
...
const confirmDeleteModel = (model: typeof customModels.value[0]) => {
...
const selectProvider = (provider: typeof providerList[0]) => {
...
const handleClickOutside = (event: MouseEvent) => {
  ...
  if (showModelDropdown.value &&
      modelSelectRef.value && !modelSelectRef.value.contains(event.target as Node) &&
      modelDropdownRef.value && !modelDropdownRef.value.contains(event.target as Node)) {
    showModelDropdown.value = false;
  }
};
